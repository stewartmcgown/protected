...
export function defaultAttemptUpdateOptions(): AttemptUpdateOptions {
    return {
        fail: false,
        ruleArgs: {},
    };
 ...
 */
export function attemptUpdate(target: object, update: object, options: AttemptUpdateOptions = defaultAttemptUpdateOptions()): ProtectedFailure[] {
    const errors = [];
    ...
    Object.entries(update).forEach(updateEntries => {
        ...
        if (meta !== undefined && meta !== null) {
            ...
            Object.entries(meta).forEach(metaEntries => {
                ...
                const dataArgs = options.ruleArgs[ruleKey];

                if (!rule.test(updateValue, target[propertyKey], ruleArg, dataArgs)) {
                    return fail({
                        target,
    ...
    if (options.fail && errors.length > 0) { throw errors; }
